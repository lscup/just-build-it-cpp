...
1. Build and run v0.3 and v0.4 side by side with the same input. List three specific differences in the output and say which is clearer and why. That judgment is design, not coding.
2. Run v0.4 with points possible 0 and confirm the explanation appears.
3. Change the report to right-align the numbers while keeping the labels left-aligned.
4. Add "Bonus points are added to points earned, not to points possible" as a note printed under the report — putting the Chapter 4 policy in front of the user.
