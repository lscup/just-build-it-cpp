...
and rebuild after each step — don't wait until the end to test. Where a step
**replaces** something from v2.1, the old code is named so you know what to
remove.
...
2. Work through Steps 1–7 above in your local editor, in order. After Steps
   1–4 (once you have `Gradebook`, `splitCsv`, `saveGradebook`, and
   `loadGradebook` in place), rebuild even though `main` hasn't changed yet —
   the file should still compile:

   ```bash
   g++ -std=c++17 -Wall -Wextra *.cpp -o gradecalc
   ```

3. Apply Steps 5 and 6 together, since `main` calls `printReport`. Rebuild
   again:
