...
2. Work through Steps 1–4 above in your local editor, in order: seed and fix
   D1/D3 in `letterFor` first, then seed and fix D2 in `percentageOf`, then
   add the `check`/`runTests` harness, then rewire the menu. Rebuild with
   debug information after each step:

   ```bash
   g++ -std=c++17 -Wall -Wextra -g *.cpp -o gradecalc
   ```

3. Use your debugger of choice (or the debugger workflow described in this
   chapter) to set breakpoints and watch the variables named in each step.
