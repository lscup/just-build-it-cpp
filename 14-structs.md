...
4. Work through Steps 1–7 above in StudySite's internal editor, in order.
   Click **Run** after Steps 1–5 (once you have the four structs, the
   scale functions returning a vector, the parameterized `letterFor`, and
   `studentPercentage` in place, and `showAbout` deleted) even though
   `main`'s entry loops haven't changed yet — the file should still compile.
   Then apply Steps 6 and 7 together, since the rewritten report loop calls
   `studentPercentage` and the new `letterFor`.
...
- **COSC1437xxx-Grade-Calculator-YourLastName** contains the Chapter 14
  checkpoint.
