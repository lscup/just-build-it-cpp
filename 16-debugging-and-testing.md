...
You will be given `broken.cpp` — a build of the Grade Calculator containing **three seeded defects**. This is the only intentionally broken code in the book, it is clearly labelled, and it is distributed separately from your own project.
...
**For each defect, work through Section 16.2's six steps and produce a defect report.**

**Defect 1 — the boundary.**
Set a breakpoint inside `letterFor`. Add a watch on the comparison being evaluated. Step through with a percentage of exactly 90.0 and observe which tier is selected. Predict, before you look, what the comparison should evaluate to.

**Defect 2 — the arithmetic.**
Hand-compute the expected percentage for 8 earned + 2 bonus out of 10. Then break inside `percentageOf` and inspect `earned` and `possible` after the accumulation loop. One of them will not be what you expect. Which, and why?

**Defect 3 — the out-of-range read.**
This one may not crash. Set a **conditional** breakpoint inside `letterFor` with the condition `i >= scale.size()`. If your belief that the loop stays in range is correct, it will never fire. Run it and see.

When it fires, read the call stack — Figure 16.1 shows what you will find.

### Then

1. **Fix all three** in your own copy, one at a time, rebuilding and retesting after each.

2. **Write a regression test for each**, using the `check` harness from Section 16.7. Add a menu option to run them.

3. **Write three defect reports** using the six fields from Section 16.8.
