...
1. Add sorting and searching to your v2.3.

2. **Trace selection sort by hand** on five students, writing the roster after each pass. Then add diagnostic output printing the roster each pass and compare with your trace. Any disagreement means one of you is wrong — find out which.

3. **Break the binary search precondition deliberately.** Search for a student by ID *without* sorting first. Does it find them? Try several IDs. Explain in writing why this is worse than a slow search.

4. Add a fourth ordering — by letter grade, then by name within each grade. Which functions did you change?

5. **Measure at scale.** Generate 1,000 students with sequential IDs and repeat the search comparison. Now which is cheaper, and by how much? At what roster size did binary search start to win?

6. Replace your `selectionSort` with `std::sort` and confirm identical output. Which do you prefer, and why?
