...
2. Add a variable for bonus points and read it. Do not use it yet — Chapter 4 will.
3. Add a `const double` for the A cutoff, set to 90.0. Print it as part of the summary.
4. **Try to break it deliberately.** Move the `Points earned` prompt above the `Assignment name` prompt, rebuild, and run. What happens to the assignment name, and why? Move it back.

Step 4 is the trap in Section 3.12, produced by your own hands. That is worth five minutes.
