...
1. Build and run. Exercise every menu option, including an invalid one.

2. **Do the refactor yourself, incrementally.** Start from your v0.7. Extract `assignLetterGrade` first — move the chain into a function, replace it with a call, rebuild, run, confirm identical output. Then `computePercentage`. Then `printReport`. Then `readAssignment`.

   **Rebuild and run after each one.** If a step breaks, you know exactly which twenty lines caused it. This is Section 9.6, and doing it once properly is worth more than reading about it three times.

3. Write a small test `main` that calls `assignLetterGrade` with 90.0, 89.9, 80.0, 60.0, 59.9, and 0.0. Check each against what you expect *before* running it.

4. Add an `applyBonus` default argument of `0.0` and confirm existing calls still compile.

5. Add a fourth menu option that prints just the letter grade. How many lines did it take? Compare with what the same addition would have cost in v0.7.
