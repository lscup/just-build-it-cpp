...
1. Add saving and loading to your v2.1. Save on exit and load at startup.

2. **Write the format documentation** as a separate file, including the commas-in-fields restriction.

3. **Test the failure paths deliberately.** Each should behave sensibly:
   - Run with no `gradebook.csv` present.
   - Corrupt a number in the file — change `84` to `eighty-four`.
   - Truncate a `STUDENT` line so it has only two fields.
   - Add a line with an unknown tag, such as `HOMEWORK,1,2`.
   - Delete every `TIER` line.

4. **Verify no data is destroyed on a bad load.** Save a good gradebook, corrupt one line, restart, and confirm you still have your data in memory. Then look at what happens when you save afterward — is the corrupt line preserved or overwritten? Is that what you want?

5. **Answer the format question in writing.** Could your format store category names and weights for weighted grading? Explain how, or explain why not. Keep your answer — Chapter 21 will check it.

6. Add a menu option to save under a different filename, so an instructor can keep several sections.
